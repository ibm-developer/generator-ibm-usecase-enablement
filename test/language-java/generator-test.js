...
class Options {
	constructor(backendPlatform, skitPath) {
		const testFilePath = JSON.stringify(path.join(__dirname, '..', 'resources-java', skitPath));
		this.values = {
...
FRAMEWORKS.forEach(framework => {
	describe(`java generator : test ${framework} `, function () {
		this.timeout(10000);
		const folder = framework == 'java' ? 'liberty' : 'spring';
		const options = new Options(framework, 'skit');
		const location = path.join(__dirname, '..',  'resources-java', 'skit', 'java-' + folder, 'config.json.template');
		const expected = JSON.parse(fs.readFileSync(location));
		...
		const publicRoot = (folder == 'liberty') ? path.join('src','main','webapp') : path.join('src', 'main', 'resources', 'static');
...
FRAMEWORKS.forEach(framework => {
	describe(`java generator : test config only ${framework} `, function () {
		this.timeout(10000);
		const options = new Options(framework, 'skit-configonly');
		before(options.before.bind(options));
		const folder = framework == 'java' ? 'liberty' : 'spring';
		const location = path.join(__dirname, '..',  'resources-java', 'skit-configonly', 'java-' + folder, 'config.json.template');
		const expected = JSON.parse(fs.readFileSync(location));
		it('should have generated with expected config', function() {
			assert.deepEqual(options.actual, expected, 'Configuration did not match');
		});
